<?php
require_once modification(DIR_SYSTEM . 'library/evopayments/payments.php');
class ControllerExtensionPaymentEvopayments extends Controller {
    /**
	...
	public function index() {
		if ($order_info = $this->model_checkout_order->getOrder($this->session->data['order_id'])) {
			...
			$this->language->load('extension/payment/evopayments');
			...
			$post_data = array();
			$post_data['merchantId'] = trim($this->config->get('payment_evopayments_clientid'));
			$post_data['merchantTxId'] = substr(md5(uniqid(mt_rand(), true)), 0, 20);
			$post_data['password'] = trim($this->config->get('payment_evopayments_password'));
			$post_data['brandId'] = trim($this->config->get('payment_evopayments_brandid'));
			
			$post_data['customerId'] = $order_info['customer_id'];
			$post_data['allowOriginUrl'] = $this->getAllowOriginUrl();
			$post_data['merchantLandingPageUrl'] = html_entity_decode($this->url->link('extension/payment/evopayments/redirectBack', 'order_id='.$this->session->data['order_id'],true), ENT_QUOTES, 'UTF-8');
			$post_data['merchantNotificationUrl'] = html_entity_decode($this->url->link('extension/payment/evopayments/callback', 'order_id='.$this->session->data['order_id'],true), ENT_QUOTES, 'UTF-8');
...
// 			$post_data['customerAddressState'] = html_entity_decode($order_info['payment_zone_code'], ENT_QUOTES, 'UTF-8');
			...
			try {
			    ...
			    $payment_evopayments_pay_action = $this->config->get('payment_evopayments_pay_action'); //1 refer to auth, 0 refer to purchase
			    if ($payment_evopayments_pay_action){
			    ...
			    return $this->load->view('extension/payment/evopayments', $data);
			}
			...
			if(isset($res->result) && $res->result == 'success'){
			    ...
			    $data['merchantId'] = trim($this->config->get('payment_evopayments_clientid'));
			    
			    $data['button_confirm'] = $this->language->get('button_confirm');
			    
			    $testmode = $this->config->get('payment_evopayments_testmode');
			    if ($testmode) $data['action'] = $this->config->get('payment_evopayments_test_cashier_url');
			    else $data['action'] = $this->config->get('payment_evopayments_cashier_url');
			    
			    $data['java_script_url'] = $this->config->get('payment_evopayments_test_javascript_url');
			    $data['cashier_url'] = $this->config->get('payment_evopayments_test_cashier_url');
			}else{
			    $data['error'] = $this->language->get('text_error_message');
			}
			if($this->config->get('payment_evopayments_pay_type') == 1){
			    //iframe payment mode
			    $data['post_url'] = $post_data['merchantLandingPageUrl'];
			    return $this->load->view('extension/payment/evopayments_iframe', $data);
			}else if($this->config->get('payment_evopayments_pay_type') == 2){
	...
	private function initConfig(){
	    $this->environment_params['merchantId'] =  trim($this->config->get('payment_evopayments_clientid'));
	    $this->environment_params['password'] = trim($this->config->get('payment_evopayments_password'));
	    $testmode = $this->config->get('payment_evopayments_testmode');
	    if ($testmode){
	        $this->environment_params['tokenURL'] = $this->config->get('payment_evopayments_test_token_url');
	        $this->environment_params['paymentsURL'] = $this->config->get('payment_evopayments_test_payments_url');
	        $this->environment_params['baseUrl'] = $this->config->get('payment_evopayments_test_cashier_url');
	        $this->environment_params['jsApiUrl'] = $this->config->get('payment_evopayments_test_javascript_url');
	    }else{
	        $this->environment_params['tokenURL'] = $this->config->get('payment_evopayments_token_url');
	        $this->environment_params['paymentsURL'] = $this->config->get('payment_evopayments_payments_url');
	        $this->environment_params['baseUrl'] = $this->config->get('payment_evopayments_cashier_url');
	        $this->environment_params['jsApiUrl'] = $this->config->get('payment_evopayments_javascript_url');
	    }
	}
	...
	private function checkStatus($order_id,$merchantTxId){
	    ...
	    try {
	        ...
	        $this->load->model('checkout/order');
	        $this->load->model('extension/payment/evopayments');
	        ...
	        if(!isset($result->result) || $result->result != 'success'){
	            ...
	            if($this->model_extension_payment_evopayments->getTotalOrderHistories($order_id)){
	                //the order status has been checked already
	                $this->response->redirect($this->url->link('checkout/failure'));
	            }else{
	                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_evopayments_failed_status_id'),'Failed');
	                $this->response->redirect($this->url->link('checkout/failure'));
	            }
	            exit;
	        }else{
	            if($this->model_extension_payment_evopayments->hasOrder($order_id)){
	                //the order status has been checked already
	            ...
	            if($result->status == 'SET_FOR_CAPTURE' || $result->status == 'CAPTURED'){
	                ...
	                $evopayments_order_id = $this->model_extension_payment_evopayments->addOrder($order_data);
	                $this->model_extension_payment_evopayments->addTransaction($evopayments_order_id,'payment', $order_info);
	                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_evopayments_success_status_id'),'Paid');
	                $this->response->redirect($this->url->link('checkout/success'));
	            }else if($result->status == 'NOT_SET_FOR_CAPTURE'){
	                // AUTH was successful
	                $evopayments_order_id = $this->model_extension_payment_evopayments->addOrder($order_data);
	                $this->model_extension_payment_evopayments->addTransaction($evopayments_order_id,'auth', $order_info);
	                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_evopayments_auth_status_id'),'Authorized');
	                $this->response->redirect($this->url->link('checkout/success'));
	            }else{
